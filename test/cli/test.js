...
tap.test('test command', async (tap) => {
  ...
  tap.test('test command should call test module with branch name ', async (tap) => {
    gitFixture.init()

    const result = childProcess.execSync(`${wibyCommand} test --dependent="https://github.com/wiby-test/fakeRepo" --branch="fake"`, {
      env: {
        ...process.env,
        NODE_OPTIONS: `-r ${fixturesPath}/http/test-command-positive.js`
      }
