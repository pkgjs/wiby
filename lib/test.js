...
module.exports = async function ({ dependents }) {
  ...
  for (const { repository: url, pullRequest, branch } of dependents) {
    const dependentRepositoryInfo = gitURLParse(url)
    const dependentPkgJson = await github.getPackageJson(dependentRepositoryInfo.owner, dependentRepositoryInfo.name, branch)
    debug(`Dependent module: ${dependentRepositoryInfo.owner}/${dependentRepositoryInfo.name}, branch ${branch}`)
    ...
    await pushPatch(patchedPackageJSON, dependentRepositoryInfo.owner, dependentRepositoryInfo.name, parentPkgJSON.name, parentBranchName, branch)
    if (pullRequest) {
...
async function pushPatch (dependentPkgJson, dependentOwner, dependentRepo, parentName, parentBranchName, branchRef) {
  ...
  if (!testingBranch) {
    ...
    const { headSha, treeSha } = await github.getShas(dependentOwner, dependentRepo, branchRef)
