...
module.exports = async function ({ dependents }) {
  ...
  for (const { repository: url } of dependents) {
    ...
    const defaultBranch = await github.getDefaultBranch(dependentPkgInfo.owner, dependentPkgInfo.name)
    const dependentPkgJSON = await github.getPackageJson(dependentPkgInfo.owner, dependentPkgInfo.name, defaultBranch)
